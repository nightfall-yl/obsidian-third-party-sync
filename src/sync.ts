...
export const isPasswordOk = async (
  ...
  // Support both remotely-sync and legacy remotely-save metadata file names.
  const remoteHasAnyPlainMetadata = remote.some(
    (item) =>
      item.key === DEFAULT_FILE_NAME_FOR_METADATAONREMOTE ||
      item.key === DEFAULT_FILE_NAME_FOR_METADATAONREMOTE2 ||
      item.key === LEGACY_FILE_NAME_FOR_METADATAONREMOTE ||
      item.key === LEGACY_FILE_NAME_FOR_METADATAONREMOTE2
  );
  log.debug(
    "password is blank? ",
    password == "",
    " remote has metadata? ",
    remoteHasAnyPlainMetadata
  );
  
  if (password == "") {
    // No password: check if remote has unencrypted metadata
    if (remoteHasAnyPlainMetadata) {
      return {
        ok: true,
        reason: "no_password_both_sides"
      } as PasswordCheckType;
    }
    // No password and no metadata - could be empty vault or all files unencrypted
    // Try to find any file that could be metadata (not base64 encoded)
    const couldBeMetadata = remote.some(
      (item) => !item.key.includes(".") || item.key.endsWith(".json")
    );
    if (couldBeMetadata) {
      return {
        ok: true,
    ...
    // Can't determine, but let sync continue
    return {
      ok: true,
      reason: "no_password_both_sides"
    } as PasswordCheckType;
...
export const getRemoteMetadata = async (
  ...
  for (const entry of remote) {
    ...
    if (
      ...
      key === LEGACY_FILE_NAME_FOR_METADATAONREMOTE
    ) {
      metadataFiles.push({
        key: key,
        existRemote: true,
  ...
  // Delete older duplicate metadata files. Keep legacy/new formats if both exist,
  // and only clean up exact duplicates of the same metadata name.
...
const isSkipItem = (
  ...
  , configDir: string) => {
  const isBookmarkFile = key === configDir + FILE_NAME_FOR_BOOKMARK_FILE;
  if (isBookmarkFile) {
  ...
  // Always skip metadata files - they are managed by the plugin itself
  if (
    key === DEFAULT_FILE_NAME_FOR_METADATAONREMOTE ||
    key === DEFAULT_FILE_NAME_FOR_METADATAONREMOTE2 ||
    key === LEGACY_FILE_NAME_FOR_METADATAONREMOTE ||
    key === LEGACY_FILE_NAME_FOR_METADATAONREMOTE2
  ) {
    return true;
  }
...
const ensembleMixedStates = async (
  ...
  if (localConfigDirContents !== undefined) {
    for (const entry of localConfigDirContents) {
      const key = entry.key;
      const isBookmarkFile = key === configDir + FILE_NAME_FOR_BOOKMARK_FILE;
      // If we're not syncing the config dir, only keep bookmark file when bookmark sync is enabled.
      if (!syncConfigDir && !(syncBookmarks && isBookmarkFile)) {
        continue;
      }
...
const assignOperationToFileInplace = (
  ...
  // Check if pull-only modes should skip local changes
  const isPullOnlyMode = syncDirection === "incremental_pull_only" || syncDirection === "incremental_pull_and_delete_only";
  // Check if push-only modes should skip remote changes
  const isPushOnlyMode = syncDirection === "incremental_push_only" || syncDirection === "incremental_push_and_delete_only";

  // 1. mtimeLocal
  if (r.existLocal) {
    // For pull-only modes, ignore local changes - don't upload to remote
    if (isPullOnlyMode && (!r.existRemote || r.mtimeLocal > r.mtimeRemote)) {
      // Local has changes but we're in pull-only mode
      if (r.mtimeLocal === r.mtimeRemote && sizeLocalComp === sizeRemoteComp) {
        // Files are actually the same, skip
        r.decision = "skipUploading";
        r.decisionBranch = 50; // Pull-only: local matches remote
        keptFolder.add(getParentFolder(r.key));
        return r;
      }
      // Local is newer/missing remotely but we don't push in pull-only mode
      r.decision = "skipUploading";
      r.decisionBranch = 51; // Pull-only: skip local upload
      keptFolder.add(getParentFolder(r.key));
      return r;
    }
  ...
  // 2. mtimeRemote
  if (r.existRemote) {
    // For push-only modes, ignore remote changes - don't download from remote
    if (isPushOnlyMode && r.mtimeRemote > mtimeLocal) {
      // Remote has changes but we're in push-only mode
      if (r.mtimeLocal === r.mtimeRemote && sizeLocalComp === sizeRemoteComp) {
        // Files are actually the same, skip
        r.decision = "skipUploading";
        r.decisionBranch = 52; // Push-only: local matches remote
        keptFolder.add(getParentFolder(r.key));
        return r;
      }
      // Remote is newer but we don't pull in push-only mode
      r.decision = "skipUploading";
      r.decisionBranch = 53; // Push-only: skip remote download
      keptFolder.add(getParentFolder(r.key));
      return r;
    }
  ...
  // 3. deltimeLocal
  // Only process local deletion in push modes or bidirectional
  if (r.deltimeLocal !== undefined && r.deltimeLocal !== 0) {
    // For pull-only modes, ignore local deletions
    if (syncDirection === "incremental_pull_only") {
      // Skip local deletion, but still need to handle remote side
      r.decision = "skipUploading";
      r.decisionBranch = 54; // Pull-only: skip local deletion
      keptFolder.add(getParentFolder(r.key));
      return r;
    }
    const mtimeLocal = r.existLocal ? r.mtimeLocal : -1;
  ...
  // 4. deltimeRemote
  // Only process remote deletion in pull modes or bidirectional
  if (r.deltimeRemote !== undefined && r.deltimeRemote !== 0) {
    // For push-only modes, ignore remote deletions
    if (syncDirection === "incremental_push_only") {
      r.decision = "skipUploading";
      r.decisionBranch = 55; // Push-only: skip remote deletion
      keptFolder.add(getParentFolder(r.key));
      return r;
    }
    const mtimeLocal = r.existLocal ? r.mtimeLocal : -1;
...
export const getSyncPlan = async (
  ...
  for (let i = 0; i < sortedKeys.length; ++i) {
    ...
    if (key.endsWith("/")) {
      ...
      await assignOperationToFolderInplace(val, keptFolder, vault, password, syncDirection);
    } else {
...
export const uploadExtraMeta = async (
  ...
  // No-upload version: always skip metadata file upload
  // This matches remotely-save behavior - no _remotely-secure-metadata-on-remote.json
  log.debug(`uploadExtraMeta: skipping upload (no-metadata version)`);
  if (metadataFile !== undefined) {
    return metadataFile.mtimeRemote;
  }
  // Return current time as mtime to avoid triggering getMetadataMtime() fallback,
  // which would make an extra listFromRemote() call that may fail with 401.
  return Date.now();
};
...
const splitThreeSteps = (syncPlan: SyncPlanType, sortedKeys: string[]) => {
  ...
  return {
    ...
    realTotalCount: fileSyncCount, // Only count actual file sync (upload/download)
...
export const doActualSync = async (
  ...
  log.debug(`finish syncing extra data lastly, new metadata mtime: ${newMetadataMtime}`);
