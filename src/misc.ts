...
/**
 * Convert ArrayBuffer to hex-encoded string.
 * @param b ArrayBuffer
 * @returns hex string
 */
...
export const getRandomArrayBuffer = (byteLength: number) => {
  ...
  const k = crypto.getRandomValues(new Uint8Array(byteLength));
...
/**
 * Check if the folder name is a special folder that should be skipped
 * @param folderName
 * @returns
 */
