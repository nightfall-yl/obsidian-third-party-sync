...
export default class RemotelySavePlugin extends Plugin {
  ...
  async syncRun(triggerSource: SyncTriggerSourceType = "manual") {
    ...
    try {
      ...
      if (!passwordCheckResult.ok) {
        getNotice(triggerSource, t("syncrun_passworderr"), 10 * 1000);
        everythingOk = false;
      ...
      if (triggerSource !== "dry") {
        // Step 7 (actual sync)
        notifyFunc(triggerSource, 7);
        // doActualSync returns the mtime of the newly uploaded metadata file
        // This is more reliable than calling getMetadataMtime() which depends on cloud storage list API
        const newMetadataMtime = await this.doActualSync(client, plan, sortedKeys, metadataFile, origMetadataOnRemote, sizesGoWrong, deletions, self);
        if (newMetadataMtime !== undefined) {
          this.settings.lastSynced = newMetadataMtime;
          await this.saveSettings();
          log.debug("Updated lastSynced from upload result:", newMetadataMtime);
        } else {
          // Fallback: get from remote if upload didn't return mtime
          log.debug("start getting last synced from remote (fallback)");
          this.settings.lastSynced = await this.getMetadataMtime();
          await this.saveSettings();
          log.debug("finish getting last synced from remote (fallback)");
        }
      } else {
        // Step 7 (dry run skip)
        notifyFunc(triggerSource, 7);
      ...
      statusBarFunc(triggerSource, 8, everythingOk);

      // Step 8 (finish) - notify after status bar is switched to idle.
  ...
  private updateLastSyncMsg(lastSyncedMillis?: number) {
    ...
    if (lastSyncedMillis !== undefined && lastSyncedMillis > 0) {
      const deltaTime = Date.now() - lastSyncedMillis;
      ...
      if (seconds < 60) {
        ...
        lastSyncMsg = getLastSynced(this.i18n, lastSyncedMillis).lastSyncMsg;
      }
