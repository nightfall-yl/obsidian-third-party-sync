...
class ObsHttpHandler extends FetchHttpHandler {
  ...
  async handle(
    ...
    let transformedBody: ArrayBuffer | string = body;
...
const fromS3ObjectToRemoteItem = (x: S3ObjectType): RemoteItem => {
  if (
    x === undefined ||
    x.Key === undefined ||
    x.LastModified === undefined ||
    x.Size === undefined
  ) {
    throw Error(`invalid S3 object shape: ${JSON.stringify(x)}`);
  }
  return {
    key: x.Key,
...
export const listFromRemote = async (
  ...
  for (const x of contents) {
    try {
      unifiedContents.push(fromS3ObjectToRemoteItem(x));
    } catch (err) {
      log.warn(
        `skip unrecognized S3 object while listing remote: ${JSON.stringify({
          key: x?.Key,
        })}`,
        err
      );
